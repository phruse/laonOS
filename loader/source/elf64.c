...
elf64_section(elf64_file_t *file_ptr, uint64_t section_offset);
...
bool elf64_init_executable(uint32_t file_ptr, uint32_t file_end, elf64_t *file) {
    uint32_t file_size = file_end - file_ptr;
    uint32_t align_size = ALIGN(file_size, 64);

    if (!file->relocate) {
        file->relocate = file_ptr;
    } else {
        memcpy((void *) file->relocate,
               (const void *) file_ptr, align_size);
    }

    file->file_end = file->relocate + align_size;

    elf64_file_t *header = (elf64_file_t *) file->relocate;
    file->file_ptr = header;
    file->entry = file->relocate + header->entry;
    ...
    if (section) {
        // init .bss section
        memset((void *) (file->relocate + section->offset),
               0, section->size);
    }
...
bool elf64_relocate_executable(const elf64_t *file, uint64_t location) {
    elf64_section_t *rela_section =
            elf64_section_name(file->file_ptr, ELF_SECTION_RELA);
    elf64_section_t *dynsym_section =
            elf64_section(file->file_ptr, rela_section->link);
    ...
    uint64_t file_address = (uint64_t) file->file_ptr;
