...
inline static size_t puts__(const char *text);
...
static void putchar__(const int character) {
    if (character == '\n' || character == '\r') {// newline
        video_mem += 160 - ((uint32_t)(video_mem - VGA_VIDEO_ADDRESS) % 160);
    } else {
...
// set cursor position
static void cursor_position() {
    uint32_t position = (uint32_t)(video_mem - VGA_VIDEO_ADDRESS) / 2;
