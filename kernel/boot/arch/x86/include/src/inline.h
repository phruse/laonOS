...
#pragma once
#ifndef LAONOS_BOOT_ARCH_X86_INCLUDE_SRC_INLINE_H
#define LAONOS_BOOT_ARCH_X86_INCLUDE_SRC_INLINE_H
 ...
 */
inline static bool is_long_mod_support(void) {
  uint32_t edx, tmp_null;
  __get_cpuid(0x80000001, &tmp_null, &tmp_null, &tmp_null, &edx);
 ...
 * @return if support return true and otherwise return false
 */
inline static bool is_5_level_support(void) {
  uint32_t ecx, tmp_null;
  __get_cpuid_count(7, 0, &tmp_null, &tmp_null, &ecx, &tmp_null);
...
inline static void page_init(const uintptr_t table_address,
  ...
  asm volatile(
      ...
      "mov %[pml_table], %%eax; mov %%eax, %%cr3;" ::[pml_table] "r"(
          table_address)
...
jump_64:
  asm volatile(".code64 \n\t push %%rax; mov %1, +4(%%rsp); pop %%rax;"
               "jmp *%%rax;" ::"a"((uint32_t)(entry_address & 0xFFFFFFFFLL)),
               "b"((uint32_t)((entry_address >> 32) & 0xFFFFFFFFLL)));
}

#endif // LAONOS_BOOT_ARCH_X86_INCLUDE_SRC_INLINE_H
