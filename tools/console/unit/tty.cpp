...
public:
  explicit tty(std::string device_path)
      : device(open(device_path.c_str(), O_RDWR | O_NOCTTY | O_SYNC)),
        offset(0) {
    if (device == -1) {
  ...
  auto tty_read(std::string &&end, std::size_t max_get = 32) {
    ...
    throw call_error("read block (out of range)");
  }
  ...
  std::string_view call(call_code_e code, std::string &&option) override {
    tty_write(static_cast<char>(code) + option);
    auto receive = tty_read(std::string(TEST_SEND) + static_cast<char>(code));
